...
from carry.carry_utils import create_two_handed_carry_constraint, hand_to_track, set_carry_track_param_if_needed, holster_carried_object, interact_with_carried_object, get_carried_objects_gen, PARAM_CONTEXT_CARRY_HAND
...
from sims4.tuning.tunable import OptionalTunable, TunableTuple, TunableSimMinute, TunableSet, HasTunableReference, Tunable, TunableList, TunableVariant, TunableEnumEntry, TunableReference, TunableRange, TunableMapping, TunableThreshold, TunablePackSafeReference, TunableEnumSet
...
class Interaction(HasTunableReference, ContentScoreMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)):
    ...
    @classmethod
    def check_if_valid(cls, is_bucks):
    ...
    @classmethod
    def register_upper_limit_callback(cls, callback):
        cls._upper_limit_callback = callback

    @classmethod
    def register_cost_gain_strings_callbacks(cls, cost_string_callback, gain_string_callback):
    ...
    def get_affordance_actor_filters(sim_info):
    ...
    @staticmethod
    @caches.clearable_barebones_cache
    def get_affordance_target_filters(sim_info):
    ...
    @classmethod
    def test_affordance_filters(cls, sim, target):
    ...
    def test(cls, inst, target=DEFAULT, context=DEFAULT, super_interaction=None, skip_safe_tests=False, **interaction_parameters):
        ...
        try:
            ...
            if target_of_interest and target_of_interest.is_on_active_lot():
                if resolver is None:
                    ...
                    if funds_source == FundsSource.STATISTIC:
                        ...
                        if cost_for_source > 0:
                            ...
                            if cls.missing_funds_additional_description:
                                ...
                                unavailable_funds_tooltip = lambda *_, **__: unavailable_funds_text
                            result &= event_testing.results.TestResult(False, "Sim [{}] either has no household funds manager, or doesn't have enough funds.:", context.sim, tooltip=unavailable_funds_tooltip)
            if result and cls.utility_info is not None and result and result:
            ...
            if cls._should_test_affordance_filters(context):
                result &= cls.test_affordance_filters(context.sim, target)
    ...
    @flexmethod
    def test_for_phone_notification(cls, inst, resolver):
        if cls.phone_notification_tests is None:
            return False
        return cls.phone_notification_tests.run_tests(resolver)
    ...
    def get_participants(cls, inst, participant_type:ParticipantType, sim=DEFAULT, target=DEFAULT, carry_target=DEFAULT, listener_filtering_enabled=False, target_type=None, **interaction_parameters) -> set:
        ...
        if participant_type & ParticipantType.ObjectIngredients and target is not None and (target.is_sim or target.crafting_component):
            ...
            if target_crafting_process is not None:
                result.extend(target_crafting_process.get_ingredients_object_definitions())
        ...
        if sim is not None and hasattr(sim, 'posture_state') and target is not None and target.is_sim and hasattr(target, 'posture_state'):
            ...
            if inst is not None and inst.created_target is not None:
                ...
                if inst.created_target.crafting_component:
                    ...
                    if inst_created_target_crafting_process is not None:
                        result.extend(inst_created_target_crafting_process.get_ingredients_object_definitions())
                    ...
                    if obj is not None:
                        ...
                        if obj.crafting_component:
                            ...
                            if crafting_process is not None:
                                result.extend(crafting_process.get_ingredients_object_definitions())
                    else:
    ...
    @classmethod
    def get_upper_limit_on_payment(cls, funds_source, context=DEFAULT):
    ...
    @flexmethod
    def get_cost_name_factory(cls, inst):
        inst_or_cls = inst if inst is not None else cls
    ...
    @flexmethod
    def get_gain_name_factory(cls, inst):
        inst_or_cls = inst if inst is not None else cls
    ...
    def constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor):
        ...
        allow_multi_surface = False
        overrides_multi_surface_constraints = None
        overrides_multi_surface_constraints = target.override_multi_surface_constraints
        allow_multi_surface = True
    ...
    @classmethod
    def _constraint_gen(cls, sim, target, participant_type=ParticipantType.Actor, interaction=None):
    ...
    def get_constraint_for_slave_routing(self, sim, target, constraint):
    ...
    def constraint_intersection(cls, inst, sim=DEFAULT, target=DEFAULT, participant_type=DEFAULT, *, posture_state=DEFAULT, force_concrete=True, allow_holster=DEFAULT, invalid_expected=False):
        ...
        if target is None or target.override_multi_surface_constraints is not False:
            final_intersection = final_intersection.get_multi_surface_version()
        if inst_or_cls._multi_surface and inst is not None:
            if allow_holster is DEFAULT:
    ...
    def remove_liability(self, key, release=True):
    ...
    def build_item_cost_element(self, sequence):
    ...
    def push_tunable_continuation(self, tunable_continuation, multi_push=True, insert_strategy=QueueInsertStrategy.NEXT, actor=DEFAULT, notify_continuation=False, **kwargs):
    ...
    def _avoid_participants_as_body_target_dest_test(self, body_target):
    ...
    def add_preferred_body_target_participants(self):
    ...
    def should_show_route_fail(self):
    ...
    def register_on_path_planned_callback(self, callback):
    ...
    def unregister_on_path_planned_callback(self, callback):
    ...
    @flexmethod
    def get_interaction_reservation_handler(cls, inst, sim=DEFAULT, target=DEFAULT, resolver=None):
        inst_or_cls = inst if inst is not None else cls
    ...
    @caches.cached
    def may_reserve_target(self, sim, target):
    ...
    @property
    def telemetry_finishing_type(self):
    ...
    @property
    def should_disable_vehicles(self):
