...
class CanSeeObjectTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def __call__(self, sim_info_list=None, target_list=None):
        ...
        for sim_info in sim_info_list:
            ...
            for obj_or_sim_info in target_list:
                if obj_or_sim_info.is_sim:
                    obj = obj_or_sim_info.get_sim_instance()
                    if obj is None:
                        return TestResult(False, '{} is not instanced..'.format(obj_or_sim_info), tooltip=self.tooltip)
                else:
                    obj = obj_or_sim_info
