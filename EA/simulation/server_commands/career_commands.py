...
def select_career(sim_id:int=None, career_instance_id:int=None, track_id:int=None, level:int=None, company_name_hash:int=None, reason:int=CareerOps.JOIN_CAREER, schedule_shift_type:CareerShiftType=CareerShiftType.ALL_DAY, _connection=None):
    ...
    if reason == CareerOps.QUIT_CAREER:
        career_tracker.remove_career(career_instance_id, test_event_origin=TestEventCareersOrigin.QUIT_CAREER)
...
def set_career_lay_off_enabled(enabled:bool=True, _connection=None):
    ...
    services.get_event_manager().process_event(TestEvent.TestedGameOptionChanged, custom_keys=(TestableGameOptions.CAREER_LAY_OFF_ENABLED,))
    return True
