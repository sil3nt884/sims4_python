...
class Sim(HasSimInfoMixin, LockoutMixin, EnvironmentScoreMixin, GameObject):
    ...
    def _anim_overrides_internal(self):
        params = {'sex': self.gender.animation_gender_param, 'age': self.age.animation_age_param, 'mood': self.get_mood_animation_param_name(), 'species': SpeciesExtended.get_animation_species_param(self.extended_species)}
    ...
    def get_vehicles_for_path(self, path):
        ...
        if favorites_tracker is not None and favorite_vehicle is not None:
    ...
    def validate_location(self, location, from_reset=False):
        ...
        if routing_location.routing_surface.type == routing.SurfaceType.SURFACETYPE_POOL:
            if build_buy.is_location_pool(location.transform.translation, location.level) or wading_interval is None or water_depth <= wading_interval.upper_bound:
                return False
        ...
        if from_reset or not (on_object_surface and self.posture.unconstrained):
            agent_radius = routing.FAKE_AGENT_RADIUS_FOR_OBJECT_ROUTING_SURFACE_VALIDATION
        else:
    ...
    def validate_current_location_or_fgl(self, from_reset=False):
        ...
        zone = services.current_zone()
        if not (self.can_swim() or self.should_be_swimming_at_position(self.position, self.location.level, check_can_swim=False)):
            if self.routing_surface.type == SurfaceType.SURFACETYPE_OBJECT:
                ...
                return
        if (zone.is_in_build_buy or from_reset or from_reset) and zone.is_in_build_buy:
        ...
        def get_reset_location():
            fgl_context = placement.FindGoodLocationContext(starting_location, ignored_object_ids=ignored_object_ids, additional_avoid_sim_radius=routing.get_sim_extra_clearance_distance(), search_flags=search_flags, routing_context=self.routing_context, water_depth_info=water_depth_info)
    ...
    def get_location_on_nearest_surface_below(self):
        if self.posture_state.valid and not (self.posture.constrained_by_object or self.posture.consider_constrained_for_on_surface_checks or self.active_transition is not None and self.active_transition.source.constrained_by_object):
    ...
    def _potential_behavior_affordances_gen(self, context, **kwargs):
        ...
        for affordance in self.sim_info.career_tracker.get_cached_super_affordances_gen():
            if self._can_show_affordance(shift_held, affordance):
                yield affordance
        if context is not None and self.sim_info.unlock_tracker is not None and self.sim_info.career_tracker is not None and self.sim_info.whim_tracker is not None:
    ...
    def _potential_interactions_internal(self, context, get_interaction_parameters=None, **kwargs):
        ...
        for relbit in context.sim.sim_info.relationship_tracker.get_all_bits(self.sim_id):
            for affordance in relbit.super_affordances:
                yield from self._get_interactions_gen(context, get_interaction_parameters, affordance, **kwargs)
        if context.sim is not None and context.sim is not None:
        ...
        if context.source == InteractionSource.AUTONOMY and context.sim is self and context.sim is not self:
    ...
    def locked_from_obj_by_privacy(self, obj):
        for privacy in services.privacy_service().privacy_instances:
            if not privacy.privacy_violators & PrivacyViolators.SIM:
                pass
            elif self in privacy.allowed_sims:
                pass
    ...
    def on_add(self):
        ...
        if household is not None and household.scenario_tracker is not None:
    ...
    def _update_face_and_posture_gen(self, timeline, can_be_carried=False):
        ...
        try:
            ...
            if self._should_be_swimming():
                ...
                elif self.posture_state.body_target.is_sim:
    ...
    def _startup_sim_gen(self, timeline):
        ...
        try:
            ...
            if self._simulation_state == SimulationState.INITIALIZING:
                ...
                if self.sim_info.developmental_milestone_tracker is not None and services.current_zone().is_zone_running:
                    self.sim_info.developmental_milestone_tracker.start_milestone_tracker()
                self.update_sleep_schedule()
                if self.sim_info.time_sim_was_saved is None and self.sim_info.degree_tracker is not None and self.sim_info.degree_tracker.get_enrolled_major() is not None:
    ...
    def create_posture_interaction_context(self, hand=None):
        return InteractionContext(self, InteractionContext.SOURCE_POSTURE_GRAPH, Priority.High, carry_hand=hand)
    ...
    def posture_state(self, value):
        ...
        if self._posture_state is not None and front_carry_target_found:
    ...
    def evaluate_si_state_and_cancel_incompatible(self, finishing_type, cancel_reason_msg):
        ...
        if slave_routing or parent is None or parent.routing_component is None:
    ...
    def running_interactions_with_target_gen(self, target):
        if self.queue is not None and (self.queue.running is not None and self.queue.running.is_super) and self.queue.running.target == target:
        ...
        for si in self.si_state:
            ...
            for potential_target in potential_targets:
                ...
                if potential_target.is_part and potential_target.part_owner is target:
                    break
            content_set = autonomy.content_sets.generate_content_set(self, si.super_affordance, si, context, potential_targets=(target,), scoring_gsi_handler=scoring_gsi_handler, check_posture_compatibility=True, include_failed_aops_with_tooltip=True, aop_kwargs=si.aop.interaction_parameters)
