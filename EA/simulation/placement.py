...
def _get_nearby_items_gen(position, surface_id, radius=None, exclude=None, flags=sims4.geometry.ObjectQuadTreeQueryFlag.NONE, *, query_filter):
    ...
    position_2d = sims4.math.Vector2(position.x, position.z) if radius is None and isinstance(position, sims4.math.Vector3) else position
...
def get_nearby_sims_gen(position, surface_id, radius=None, exclude=None, flags=sims4.geometry.ObjectQuadTreeQueryFlag.NONE, only_sim_position=False, only_sim_intended_position=False, include_bassinets=False):
    ...
    for obj in _get_nearby_items_gen(position=position, surface_id=surface_id, radius=radius, exclude=exclude, flags=flags, query_filter=query_filter):
        if (obj.is_sim or include_bassinets) and obj.is_bassinet:
            yield obj
...
FGL_DEFAULT_POSITION_INCREMENT = 0.3FGL_FOOTPRINT_POSITION_INCREMENT_MULTIPLIER = 0.5PositionIncrementInfo = namedtuple('PositionIncrementInfo', ('position_increment', 'from_exception'))RaytestInfo = namedtuple('RaytestInfo', ('raytest_start_offset', 'raytest_end_offset', 'raytest_radius', 'raytest_ignore_flags', 'raytest_start_point_override'), defaults=[0.0, 0.0, 0.0, 0, None])WaterDepthInfo = namedtuple('WaterDepthInfo', ('max_water_depth', 'min_water_depth', 'max_pond_water_depth', 'min_pond_water_depth'), defaults=[None, None, None, None])OffsetInfo = namedtuple('OffsetInfo', ('offset_distance', 'offset_restrictions'), defaults=[0.0, None])
class FindGoodLocationContext:
    ...
    def __init__(self, starting_routing_location, object_id=None, object_def_id=None, object_def_state_index=None, object_footprints=None, object_polygons=None, routing_context=None, ignored_object_ids=None, min_distance=None, max_distance=None, position_increment_info=None, additional_avoid_sim_radius=0, restrictions=None, scoring_functions=None, offset_info=None, random_range_weighting=None, max_results=0, max_steps=1, height_tolerance=None, terrain_tags=None, raytest_info=None, search_flags=FGLSearchFlagsDefault, water_depth_info=None, connectivity_group_override_point=None, min_head_room=None, **kwargs):
        ...
        self.set_object_id_info(object_id, object_def_id, object_def_state_index, should_test_buildbuy=search_flags & FGLSearchFlag.SHOULD_TEST_BUILDBUY)
        ...
        if gsi_handlers.routing_handlers.FGL_archiver.enabled:
            self.additional_gsi_values = {'starting_routing_location': starting_routing_location, 'max_results': max_results, 'max_steps': max_steps, 'min_distance': min_distance, 'max_distance': max_distance, 'min_head_room': min_head_room, 'additional_avoid_sim_radius': additional_avoid_sim_radius, 'random_range_weighting': random_range_weighting, 'height_tolerance': height_tolerance, 'terrain_tags': terrain_tags, 'connectivity_group_override_point': connectivity_group_override_point, 'position_increment_info': position_increment_info}
    ...
    def set_object_id_info(self, object_id=None, object_def_id=None, object_def_state_index=None, should_test_buildbuy=False):
        if should_test_buildbuy or object_id is not None or object_def_id is not None:
            logger.warn('Passing object_id: {}, object_def_id: {} to FGLContext without SHOULD_TEST_BUILDBUY flag. These fields will not be used.', object_id, object_def_id, owner='msundaram')
    ...
    def get_result_locations_for_gsi_gen(self):
        for result in self.search.get_results():
            yield (result.location, result.score, FGLSearchType(result.search_type))

    def get_bb_args_gsi_entry(self):
        values = [{'name': 'Should Test Build/Buy', 'value': str(self.search_strategy.should_test_buildbuy)}, {'name': 'object_id', 'value': str(self.search_strategy.object_id)}, {'name': 'object_def_id', 'value': str(self.search_strategy.object_def_id)}, {'name': 'object_def_state_index', 'value': str(self.search_strategy.object_def_state_index)}]
        return values
    ...
    def find_good_location(self):
        ...
        elif search_result == FGLSearchResult.FAIL_NO_GOALS_FOUND:
        ...
        return (None, None, result_message)
...
def create_fgl_context_for_object(starting_location, obj_to_place, search_flags=FGLSearchFlagsDefault, test_buildbuy_allowed=True, **kwargs):
    ...
    return FindGoodLocationContext(starting_location, object_id=obj_to_place.id if should_test_buildbuy else None, object_footprints=(footprint,) if footprint is not None else None, search_flags=search_flags, **kwargs)

def create_fgl_context_for_sim(starting_location:'Location', sim_to_place:'Union[SimInfo, Sim]', search_flags:'FGLSearchFlag'=FGLSearchFlagsDefaultForSim, water_depth_info:'WaterDepthInfo'=None, **kwargs) -> 'FindGoodLocationContext':
    object_id = None
    ...
    return FindGoodLocationContext(starting_location, object_id=object_id, search_flags=search_flags, water_depth_info=water_depth_info, **kwargs)
