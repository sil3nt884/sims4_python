...
class HouseholdTemplate(HasTunableReference, metaclass=sims4.tuning.instances.TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SIM_TEMPLATE)):
    ...
    @flexmethod
    def create_household(cls, inst, zone_id, account=None, sim_creator=None, sim_name_type=SimNameType.DEFAULT, creation_source:str='household_template', sim_debug_name:str='', household_change_origin=HouseholdChangeOrigin.TEMPLATE):
        inst_or_cls = inst if inst is not None else cls
        ...
        (created_sim_infos, household) = SimSpawner.create_sim_infos(sim_creators, zone_id=home_zone_id, account=account, starting_funds=cls._household_funds, sim_name_type=sim_name_type, creation_source=creation_source, debug_name=sim_debug_name, household_change_origin=household_change_origin)
