    ...
    def _apply_to_career(self, sim_info, career, resolver):
        for op in self.operations:
            if op.operation_type == CareerLootOp.OP_PERFORMANCE:
                ...
                career.fire(show_notification=op.show_notification)
                if op.clear_history:
                    sim_info.career_tracker.clear_career_history(career.guid64, career.current_track_tuning.guid64)
                    if op.operation_type == CareerLootOp.OP_QUIT:
                        career.quit_career(post_quit_msg=op.show_notification)
                    elif op.operation_type == CareerLootOp.OP_LAY_OFF:
                        career.lay_off(post_lay_off_msg=op.show_notification)
