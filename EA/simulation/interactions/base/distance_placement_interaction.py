...
class DistancePlacementMixin:
    ...
    def _update_water_depth_requirements(self, sim_a, sim_b, interval, **kwargs):
        water_depth_info = kwargs['water_depth_info'] if 'water_depth_info' in kwargs else None
        min_water_depth = water_depth_info.min_water_depth if water_depth_info else None
        max_water_depth = water_depth_info.max_water_depth if water_depth_info else None
        ...
        if water_depth_info is None and (min_water_depth is not None or max_water_depth is not None):
            water_depth_info = WaterDepthInfo(max_water_depth=max_water_depth, min_water_depth=min_water_depth)
        elif water_depth_info is not None:
            water_depth_info = water_depth_info._replace(max_water_depth=max_water_depth, min_water_depth=min_water_depth)
        kwargs['water_depth_info'] = water_depth_info
        return kwargs
    ...
    def _entered_pipeline(self):
        if self.carry_target is not None:
            ...
            raytest_info = RaytestInfo(raytest_radius=self.raytest_radius, raytest_start_offset=self.raytest_offset.lower_bound, raytest_end_offset=self.raytest_offset.upper_bound)
            fgl_kwargs = {'raytest_info': raytest_info, 'ignored_object_ids': {sim.id for sim in self.required_sims()}, 'positioning_type': JigPositioning.RelativeToSimA}
