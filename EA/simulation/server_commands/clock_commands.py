...
@sims4.commands.Command('clock.setspeed_internal', command_type=CommandType.Live)
def set_speed_internal(speed:int=1, _connection=None):
    game_clock_service = services.game_clock_service()
    game_clock_service.set_clock_speed(ClockSpeedMode(speed), source=GameSpeedChangeSource.GAMEPLAY, reason='From Command', immediate=True)
    send_clock_telemetry_data(_connection)
