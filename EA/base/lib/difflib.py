...
class SequenceMatcher:
            ...
            for j in b2j.get(a[i], nothing):
                if j < blo:
                    ...
                    if k > bestsize:
                        ...
                        bestsize = k
            j2len = newj2len
        while besti > alo and bestj > blo and (isbjunk(b[bestj - 1]) or a[besti - 1] == b[bestj - 1]):
            besti = besti - 1
            bestj = bestj - 1
            bestsize = bestsize + 1
        while besti + bestsize < ahi and bestj + bestsize < bhi and (isbjunk(b[bestj + bestsize]) or a[besti + bestsize] == b[bestj + bestsize]):
            bestsize += 1
    ...
    def get_grouped_opcodes(self, n=3):
        ...
        if not ((codes or codes[0][0] == 'equal') and codes[-1][0] == 'equal' and group and len(group) == 1 and group[0][0] == 'equal'):
            yield group
...
def get_close_matches(word, possibilities, n=3, cutoff=0.6):
    ...
    if not (0.0 <= cutoff and cutoff <= 1.0):
            ...
            for i in range(alo, ahi):
                ...
                if ai == bj:
                    if eqi is None:
                        ...
                        if not cruncher.real_quick_ratio() > best_ratio or not cruncher.quick_ratio() > best_ratio or cruncher.ratio() > best_ratio:
    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        for (tag, i1, i2, j1, j2) in group:
            if tag == 'equal':
                ...
                for line in a[i1:i2]:
                    yield '-' + line
                if tag in frozenset({'delete', 'replace'}) and tag in frozenset({'insert', 'replace'}):
                    for line in b[j1:j2]:
                        yield '+' + line
...
def context_diff(a, b, fromfile='', tofile='', fromfiledate='', tofiledate='', n=3, lineterm='\n'):
    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        if not any(tag in frozenset({'delete', 'replace'}) for (tag, _, _, _, _) in group) or any(tag in frozenset({'insert', 'replace'}) for (tag, _, _, _, _) in group):
