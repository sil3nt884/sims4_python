...
class RoommateService(Service):
        ...
        if zone_id in self._zone_to_roommates:
            ...
            if household is not None:
                ...
                if travel_group is not None:
                    alternate_available = travel_group.HOUSEHOLD_AND_GUEST_MAXIMUM - total_sims - len(travel_group)
                    available_roommate_space = min(alternate_available, available_roommate_space)
            leaving_roommates = []
    ...
    def get_roommate_count(self, zone_id):
        if zone_id not in self._zone_to_roommates:
            return 0
        return len(self._zone_to_roommates[zone_id].sim_ids)
    ...
    def _remove_roommate(self, sim_id):
        ...
        travel_group = services.travel_group_manager().get_travel_group_by_zone_id(zone_id)
        if travel_group is not None:
            travel_group.object_preference_Tracker.clear_sim_restriction(sim_id)
        household = self._get_household_from_zone_id(zone_id)
        if household is not None:
    ...
    def _get_unassigned_household_count(self, household, beds, zone_id):
        ...
        for sim_info in household:
            if sim_info.is_toddler_or_younger:
                ...
                (object_id, _) = object_preference_tracker.get_restricted_object(sim_info.sim_id, self.BED_PREFERENCE_TAG, disable_overrides=True)
    ...
    def _populate_roommate_bed_assignments(self, household, beds, zone_id):
        ...
        if zone_id in self._zone_to_roommates:
            for sim_id in self._zone_to_roommates[zone_id].sim_ids:
                (object_id, _) = object_preference_tracker.get_restricted_object(sim_id, self.BED_PREFERENCE_TAG, disable_overrides=True)
                if object_id is not None:
                    obj = object_manager.get(object_id)
                    if obj is not None:
                        if obj in beds:
    ...
    def get_available_roommate_count(self, beds=None, remove_extra=False):
        ...
        travel_group = services.travel_group_manager().get_travel_group_by_zone_id(zone_id)
        if travel_group is not None:
            alternate_max_possible_new = travel_group.HOUSEHOLD_AND_GUEST_MAXIMUM - len(travel_group) - len(household)
            max_possible_new = min(max_possible_new, alternate_max_possible_new)
        if zone_id in self._zone_to_roommates:
    ...
    def copy_new_bed_assignments(self, object_preference_tracker, zone_id):
        if zone_id not in self._zone_to_roommates:
            return
        household = self._get_household_from_zone_id(zone_id)
        if household is None:
            return
        household.object_preference_tracker.safe_copy_restricted_objects_for_sim_ids(object_preference_tracker, self._zone_to_roommates[zone_id].sim_ids, self.BED_PREFERENCE_TAG, zone_id)
    ...
    def do_decorations(self):
        ...
        for (sim_id, sim_data) in household_data.sim_datas.items():
            (bed_id, _) = object_preference_tracker.get_restricted_object(sim_id, self.BED_PREFERENCE_TAG, disable_overrides=True)
