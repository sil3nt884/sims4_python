...
class OrganizationService(Service):
            ...
            for sim_id in sims:
                ...
                if sim is None:
                    ...
                    if organization_tracker is None:
                        self.remove_organization_member(sim, org_id)
                    elif organization_tracker.get_organization_status(org_id) == OrganizationStatusEnum.ACTIVE:
    ...
    def _reschedule_festival_org_event(self, drama_node, from_shutdown=False):
        ...
        if from_shutdown and services.time_service().sim_timeline.is_frozen:
            return
        setup_kwargs = {}
        if is_scoring_archive_enabled():
            gsi_data = GSIDramaScoringData()
