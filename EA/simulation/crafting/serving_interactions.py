...
class ServeObjectToCustomerMixin(ServeObjectMixin):
    ...
    def _build_sequence_with_callback(self, callback=None, sequence=()):
        ...
        carry_element_helper = CarryElementHelper(interaction=self, carry_target=self.object_to_serve, create_owning_si_fn=create_si, callback=callback, sim=self.order_sim, carry_track=self.object_info.carry_track, sequence=sequence)
...
class CreateAndServeObjectSuperInteraction(CraftingPhaseCreateObjectSuperInteraction):
    ...
    def setup_asm_default(self, asm, *args, **kwargs):
        ...
        if old_served_object is not None and old_served_object is not self.object_to_serve:
