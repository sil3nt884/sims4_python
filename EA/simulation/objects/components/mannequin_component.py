...
class MannequinComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=MANNEQUIN_COMPONENT, persistence_key=SimObjectAttributes_pb2.PersistenceMaster.PersistableData.MannequinComponent):
    ...
    @distributor.fields.ComponentField(op=distributor.ops.SetMessy)
    def mannequin_is_messy_face(self):
        return self._is_messy_face

    @mannequin_is_messy_face.setter
    def mannequin_is_messy_face(self, value):
        self._is_messy_face = value
    ...
    def on_state_changed(self, state, old_value, new_value, from_init):
        ...
        if new_appearance_modifier is not None:
            self._sim_info_data.appearance_tracker.add_appearance_modifiers(new_appearance_modifier.appearance_modifiers, state, new_appearance_modifier.priority, new_appearance_modifier.apply_to_all_outfits, new_appearance_modifier.additional_flags, source=self)
        ...
        if state is self.state_trigger_messy_face.state:
            if new_value is self.state_trigger_messy_face:
                self.mannequin_is_messy_face = True
            else:
                self.mannequin_is_messy_face = False
        ...
        for (outfit_category, outfit_list) in sim_info_data.get_all_outfits():
            if outfit_category not in REGULAR_OUTFIT_CATEGORIES:
                ...
                for (outfit_index, outfit_data) in enumerate(outfit_list):
                    ...
                    self._sim_info_data.generate_merged_outfit(sim_info_data, destination_outfit, default_outfit, source_outfit, preserve_outfit_flags=True, from_reconcile_mannequin=from_reconcile_mannequin)
                            ...
                            for (outfit_index, outfit_data) in enumerate(outfit_list):
                                if mannequin_data.is_generated_outfit_duplicate_in_category(self._sim_info_data, (outfit_category, outfit_index)):
                                    ...
                                    if outfits_in_category is not None and len(outfits_in_category) >= get_maximum_outfits_for_category(outfit_category):
                                        ...
                                        mannequin_data.generate_merged_outfit(self._sim_info_data, mannequin_data.add_outfit(outfit_category, outfit_data), mannequin_data.get_current_outfit(), (outfit_category, outfit_index), preserve_outfit_flags=True, from_reconcile_mannequin=True)
                mannequin_group_data.reconcile_mannequin_data()
                return
            if self.owner.id:
