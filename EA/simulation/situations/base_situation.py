...
class BaseSituation(IBouncerClient, SituationScoringMixin):
    ...
    def on_remove(self):
        ...
        self._stage = SituationStage.DYING
        if self.should_give_rewards and services.current_zone().is_zone_shutting_down == False:
    ...
    def _create_standard_save_seed(self):
        ...
        seed = SituationSeed(type(self), SeedPurpose.PERSISTENCE, self.id, guest_list, self.is_user_facing, duration_override=self._get_remaining_time_in_minutes(), zone_id=services.current_zone().id, start_time=self._start_time, scoring_enabled=self.scoring_enabled, main_goal_visiblity=self._main_goal_visibility, linked_sim_id=self.linked_sim_id, guest_attire_color=self.guest_attire_color, guest_attire_style=self.guest_attire_style)
    ...
    def _on_remove_sim_from_situation(self, sim):
        ...
        if sim_job.user_facing_sim_headline_display_override:
            ...
            for situation in services.get_zone_situation_manager().get_situations_sim_is_in(sim):
                if situation is self:
                    pass
                else:
                    situation.update_job_headline(sim)
    ...
    def set_job_uniform(self, sim, job, outfit_for_clothing_change=None):
        ...
        if not found_outfit:
            return
        if outfit_extra_tag_set is not None and use_existing_outfit:
    ...
    def update_job_headline(self, sim):
        sim_job = self.get_current_job_for_sim(sim)
        if sim_job is not None:
            self._resolve_sim_job_headline(sim, sim_job)
