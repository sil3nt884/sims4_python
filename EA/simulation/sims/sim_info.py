...
class SimInfo(SimInfoWithOccultTracker, SimInfoCreationSource.SimInfoCreationSourceMixin, AgingMixin, PregnancyClientMixin, SimInfoFavoriteMixin, ComponentContainer, HasStatisticComponent):
            ...
            try:
                ...
                if trans is None:
                    ...
                    if spawn_point is not None:
                        ...
                        (trans, orient, _) = self._find_place_on_lot_for_sim()
    ...
    def _load_sim_info(self, sim_proto, skip_load, is_clone=False):
        ...
        if False and (self.full_name_key != 0 or self.first_name_key != 0 or self.last_name_key != 0):
            ...
            if sim_template is not None:
                debug_name = sim_template.__name__
                if debug_name.startswith('template_'):
                    debug_name = debug_name[9:]
                setattr(self, 'debug_name', debug_name)
        if sim_attribute_data is not None:
        ...
        if not self.is_toddler_or_younger:
            ...
            for aspiration_track in aspiration_track_manager.types.values():
                if aspiration_track.is_hidden_unlockable or aspiration_track.is_valid_for_sim(self):
        ...
        if self._primary_aspiration is None or self._primary_aspiration.is_available() or self.is_human and skip_load or self._away_action_tracker is not None:
