...
class SegmentedPath:
    ...
    def generate_right_paths(self, sim, path_left):
        ...
        paths_right = self.posture_graph._right_path_gen(self.sim, self.interaction, self._distance_estimator, left_destinations, self.destinations, self.var_map, self.constraint, self.valid_edge_test, path_left, allow_carried=allow_carried, ensemble=ensemble)
...
def _shortest_path_gen(sim, sources, destinations, force_carry_path, *args, **kwargs):
    ...
    def is_destination(node):
        ...
        if force_carry_path:
            if prev and prev.body_target and prev.body_target.is_sim:
...
def _cache_global_sim_default_values():
    global SIM_DEFAULT_POSTURE_TYPE, STAND_AT_NONE, STAND_AT_NONE_CARRY, STAND_AT_NONE_NODES, SIM_DEFAULT_AOPS, SIM_TESTED_AOPS, SIM_DEFAULT_OPERATION, SIM_INFANT_OVERRIDE_DEFAULT_OPERATION, SIM_SWIM_POSTURE_TYPE, SWIM_AT_NONE, SWIM_AT_NONE_CARRY, SWIM_AT_NONE_NODES, SIM_SWIM_AOPS, SIM_SWIM_OPERATION, _MOBILE_NODES_AT_NONE, _MOBILE_NODES_AT_NONE_CARRY, _DEFAULT_MOBILE_NODES
