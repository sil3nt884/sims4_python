...
class CareerTracker(AffordanceCacheMixin, SimInfoTracker):
    ...
    def _on_confirmation_dialog_response(self, dialog, new_career, career_track=None, career_level_override=None, schedule_shift_override=CareerShiftType.ALL_DAY, disallowed_reward_types=()):
        if dialog.accepted:
            self.add_career(new_career, career_track=career_track, career_level_override=career_level_override, schedule_shift_override=schedule_shift_override, disallowed_reward_types=disallowed_reward_types)
    ...
    def add_career(self, new_career, show_confirmation_dialog=False, career_track=None, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, post_quit_msg=True, schedule_shift_override=DEFAULT, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False, allow_outfit_generation=True, show_icon_override_picker=True):
        if show_confirmation_dialog or schedule_shift_override is DEFAULT:
            (level, _, track) = new_career.get_career_entry_data(career_track=career_track, career_history=self._career_history, user_level_override=user_level_override, career_level_override=career_level_override)
            ...
            if show_confirmation_dialog:
                if self._retirement is not None:
                    self._retirement.send_dialog(Career.UNRETIRE_DIALOG, career_level_tuning.get_title(self._sim_info), icon_override=DEFAULT, on_response=lambda dialog: self._on_confirmation_dialog_response(dialog, new_career, career_track=career_track, career_level_override=career_level_override, schedule_shift_override=schedule_shift_override))
                    return
                if new_career.can_quit:
                    ...
                    if quittable_careers:
                        ...
                        career.send_career_message(switch_jobs_dialog, career_level_tuning.get_title(self._sim_info), icon_override=DEFAULT, on_response=lambda dialog: self._on_confirmation_dialog_response(dialog, new_career, career_track=career_track, career_level_override=career_level_override, schedule_shift_override=schedule_shift_override, disallowed_reward_types=(RewardType.MONEY,)))
                        return
        self.end_retirement()
    ...
    def remove_career(self, career_uid:'int', post_quit_msg:'bool'=True, update_ui:'bool'=True, test_event_origin:'TestEventCareersOrigin'=TestEventCareersOrigin.UNSPECIFIED) -> 'None':
    ...
    def remove_invalid_careers(self):
        for (career_uid, career) in list(self._careers.items()):
            if not career.is_valid_career():
                self.remove_career(career_uid, post_quit_msg=False, test_event_origin=TestEventCareersOrigin.INVALID_SIM_CAREER)
    ...
    def add_ageup_careers(self):
        for career in list(self._careers.values()):
            if career.current_level_tuning.ageup_branch_career is not None:
                ...
                self.add_career(new_career, user_level_override=career.user_level, post_quit_msg=False, schedule_shift_override=default_shift_type, show_join_msg=False, show_icon_override_picker=False)
    ...
    def get_multigig_and_standard_careers(self):
        ...
        return (standard_careers, multi_gig_careers)
    ...
    def get_quittable_careers(self, schedule_shift_type=CareerShiftType.ALL_DAY, career_category=None):
        quittable_careers = dict((uid, career) for (uid, career) in self._careers.items() if career.can_quit and career.get_is_quittable_shift(schedule_shift_type, career_category))
    ...
    def available_for_work(self, career):
        ...
        if existing_at_work_career and existing_at_work_career.guid64 != career.guid64:
            return False
        elif services.get_career_service().is_sim_info_in_pending_career_event(self._sim_info, ignorable_careers=(career,)):
            return False
        return True
    ...
    def on_death(self):
        for (uid, career) in list(self._careers.items()):
            ...
            self.remove_career(uid, post_quit_msg=False, update_ui=False, test_event_origin=TestEventCareersOrigin.DEATH)
        self.end_retirement()
            ...
            for (key, career_history) in self._career_history.items():
                if career_category is not None:
                    ...
                    if not career is None:
                        if not career.career_category == career_category:
                            pass
                        elif prev_career_key is None:
                            prev_career_key = key
                        else:
    ...
    def load(self, save_data, skip_load=False):
        ...
        for history_entry in save_data.career_history:
            if skip_load and history_entry.career_uid not in self._careers:
                ...
                if career_history is not None:
                    key = (history_entry.career_uid, history_entry.track_uid)
        ...
        for gig_history_entry in save_data.gig_history:
            ...
            if gig_history is not None:
                history_key = (gig_history.customer_id, None)
                if gig_history.gig_lot_type == DecoratorGigLotType.COMMERCIAL:
                    history_key = (None, gig_history.lot_id)
        ...
        if save_data.HasField('last_gig_history'):
            ...
            self._last_gig_history_key = (last_customer_id, last_lot_id)
    ...
    def add_gig_history(self, gig):
        ...
        new_gig_history = GigHistory(customer_id=customer_id, lot_id=lot_id, gig_id=gig.guid64, career_id=gig.career.guid64, gig_result=gig_result, gig_score=gig_score, customer_name=client_hh_name, lot_type=lot_type, project_title=project_title)
        ...
        for history in self._gig_history.values():
            ...
            if history.career_id != career.guid64:
                ...
                for (sequence_id, before_then_after_list) in gig_history_selected.items():
                    ...
                    selected_photo_sequenced_gig_history[sequence_id] = [before, after]
