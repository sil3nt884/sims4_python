...
class DaycareService(Service):
    ...
    def _apply_daycare_effects_to_sim(self, sim_info):
        ...
        elif sim_info.is_infant_or_toddler:
            daycare_interaction = self._daycare_interactions.get(sim_info)
    ...
    def _start_nanny_service(self, household, sim_infos, send_to_daycare_callback_fn):
        ...
        def _can_trigger_nanny_service(sim_info):
            if not sim_info.is_infant_or_toddler:
                return False
            else:
                daycare_interaction = self._daycare_interactions.get(sim_info)
                if daycare_interaction is not None:
    ...
    def _is_any_sim_available(self, household, residence_zone_id):
        ...
        return any(self._is_sim_available(sim_info, household, current_zone_id, residence_zone_id) for sim_info in household.caretaker_sim_info_gen())
        ...
        def _on_send_to_daycare(sim_infos):
            ...
            for sim_info in sim_infos:
                ...
                if sim_info.zone_id != current_zone_id and sim_info.away_action_tracker is not None:
                    sim_info.away_action_tracker.reset_to_default_away_action(on_travel_away=True)
            if is_active_household:
                services.client_manager().get_first_client().send_selectable_sims_update()
            ...
            for residence_zone_id in household.zone_ids_considered_residence_gen():
                if self._is_any_sim_available(household, residence_zone_id):
                    ...
                    for sim_info in tuple(daycare_sim_infos):
                        ...
                        if self.is_sim_info_at_daycare(sim_info):
                            self.remove_sim_info_from_daycare(sim_info)
                        elif sim_info in daycare_sim_infos:
                            daycare_sim_infos.remove(sim_info)
    ...
    def get_abandoned_toddlers(self, household, residence_zone_id=None, sims_infos_to_ignore=()):
        ...
        abandoned_toddlers = []
        current_zone_id = services.current_zone_id()
        for sim_info in self._potential_daycare_sims(household):
            if sim_info in sims_infos_to_ignore:
                ...
                pass
            elif not sim_info.is_infant_or_toddler:
    ...
    def get_number_of_eligible_daycare_sims(self, household):
        return sum(1 for sim_info in self._potential_daycare_sims(household) if sim_info.is_toddler_or_younger and not sim_info.is_pet)
    ...
    def refresh_household_daycare_nanny_status(self, sim_info, try_enable_if_selectable_toddler=False):
        household = services.active_household()
        if household is not None:
            try_enable = try_enable_if_selectable_toddler and (sim_info.is_infant_or_toddler and sim_info.is_selectable)
    ...
    def refresh_daycare_status(self, baby:'SimInfo', exclude_zone_id:'Optional[int]'=None) -> 'None':
        household = baby.household
        if household is None:
            return
        if baby.zone_id != exclude_zone_id and self.is_daycare_enabled(household, baby.zone_id):
    ...
    def _potential_daycare_sims(self, household):
        potential_sim_infos = list(household)
        travel_group = services.travel_group_manager().get_travel_group_by_zone_id(household.home_zone_id)
        if travel_group is not None:
            potential_sim_infos.extend(travel_group)
        return potential_sim_infos

    def is_anyone_at_daycare(self, household):
        return any(sim_info.has_trait(DaycareTuning.DAYCARE_TRAIT_ON_KIDS) for sim_info in self._potential_daycare_sims(household) if sim_info.is_toddler_or_younger)
    ...
    def send_active_household_toddlers_home(self):
        ...
        if active_household is None:
            return
        instanced_toddlers = [sim for sim in active_household.instanced_sims_gen() if sim.sim_info.is_infant_or_toddler]
