...
class ObjectCreationMixin:
    ...
    def _place_object(self, created_object):
        ...
        if not self.destroy_on_placement_failure:
            ...
            if not (participant is not None and participant.inventory_component is not None and sim is None or sim.is_sim):
...
class ObjectCreationOp(ObjectCreationMixin, BaseLootOperation):
    ...
    def __init__(self, *, creation_data, initial_states, destroy_on_placement_failure, owner_sim, location, reserve_object, temporary_tags, quantity, notification_inventory, fallback_location_target_override, require_claim, set_sim_as_owner, set_sim_as_inventory_owner, set_value_to_crafted_tooltip, spawn_vfx, **kwargs):
