...
class RelationshipBitOnFilteredSims(BaseLootOperation):
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        filter_service.submit_matching_filter(number_of_sims_to_find=self._filter_settings.desired_sim_count, sim_filter=self._filter_settings.sim_filter, callback=filter_callback, requesting_sim_info=requesting_sim, blacklist_sim_ids={subject.id}, gsi_source_fn=lambda : 'RelationshipBitOnFilteredSims Loot: Adding {} to filtered sims'.format(str(self._rel_bits)))
