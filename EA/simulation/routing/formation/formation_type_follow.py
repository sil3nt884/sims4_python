...
class FormationTypeFollow(FormationTypeBase):
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if slave_routing_component is not None:
            ...
            max_pond_water_depth = slave_routing_context.max_allowed_wading_depth
        water_depth_info = placement.WaterDepthInfo(max_water_depth=max_water_depth, min_water_depth=min_water_depth, max_pond_water_depth=max_pond_water_depth)
        fgl_kwargs.update({'water_depth_info': water_depth_info})
        ...
        if fgl_tuning.slave_should_face_master:
            ...
            raytest_info = placement.RaytestInfo(raytest_radius=self.RAYTRACE_RADIUS, raytest_start_offset=self.RAYTRACE_HEIGHT, raytest_end_offset=self.RAYTRACE_HEIGHT, raytest_start_point_override=master_location.transform.translation)
            fgl_kwargs.update({'raytest_info': raytest_info, 'ignored_object_ids': {self.master.id, self.slave.id}})
