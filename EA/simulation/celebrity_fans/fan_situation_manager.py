...
class FanSituationManager(HasTunableFactory, AutoFactoryInit):
    ...
    def _destroy_fan_situations(self):
        situation_manager = services.get_zone_situation_manager()
        for situation_id in self._fan_situation_ids:
    ...
    def _get_expected_number_of_fans(self):
        ...
        if fans_cap <= 0:
            return 0
        expected_fans = 0
        for sim in self._fannable_sims_gen():
    ...
    def _fan_count(self):
        ...
        return max(len(fan_sim_ids), len(self._fan_situation_ids) + len(self._stan_situation_ids))
    ...
    def _fannable_sims_gen(self):
        if self._celebrity_container_situation is None:
            return
        is_leaving = services.sim_spawner_service().sim_is_leaving
