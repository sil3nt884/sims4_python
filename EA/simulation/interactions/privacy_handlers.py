...
def generate_privacy_management_data(filter=None):
    ...
    for instance in privacy_service._privacy_instances:
        ...
        if (filter_list is None or ACTIVE_FILTER not in filter_list) and instance.is_active:
            ...
            entry = {'is_active': str(instance.is_active), 'max_los': str(instance._max_line_of_sight_radius), 'has_shooed': str(instance.has_shooed), 'privacy_violators': str(instance.privacy_violators), 'privacy_discouragement_cost': str(instance.privacy_discouragement_cost), 'privacy_interaction': str(instance._interaction), 'central_object': str(instance.central_object), 'min_room_area_for_discouragement': str(instance._min_room_area_for_discouragement or instance._MIN_ROOM_AREA_FOR_DISCOURAGEMENT), 'shoo_cons': str(instance._shoo_constraint_radius or instance._SHOO_CONSTRAINT_RADIUS), 'affordances': {'embarrassed_affordance': str(instance._embarrassed_affordance or instance._EMBARRASSED_AFFORDANCE), 'post_route_affordance': str(instance._post_route_affordance)}, 'allowed': {'allowed_sims': gsi_handlers.gsi_utils.format_object_list_names(instance._allowed_sims) if instance._allowed_sims else 'None', 'disallowed_sims': gsi_handlers.gsi_utils.format_object_list_names(instance._disallowed_sims) if instance._disallowed_sims else 'None', 'exempt_sims': gsi_handlers.gsi_utils.format_object_list_names(instance._exempt_sims) if instance._exempt_sims else 'None'}, 'privacy_constraints': []}
