...
class StartCraftingMixin:
    ...
    def _handle_begin_crafting(self, recipe, crafter, ordering_sim=None, crafting_target=None, orderer_ids=DEFAULT, ingredients=(), funds_source=None, paying_sim=None, ingredient_cost_only=False, situation_id=None):
        ...
        self.crafting_process = CraftingProcess(self.context.sim, crafter, recipe, discounted_price, paying_sim, reserved_ingredients=reserved_ingredients, orderer_ids=orderer_ids, original_target=original_target, ingredient_quality_bonus=avg_quality_bonus, funds_source=funds_source, bucks_cost=discounted_bucks_prices, situation_id=situation_id)
...
class StartCraftingSuperInteraction(StartCraftingMixin, PickerSuperInteraction):
    ...
    def picker_rows_gen(cls, inst, target, context, crafter=DEFAULT, order_count=1, recipe_ingredients_map=None, funds_source=None, **kwargs):
        ...
        for recipe in recipe_list:
            ...
            if (not requirements_for_recipe is not None or all_ingredients_required and ingredients_found_count < ingredients_needed_count and ingredients_needed_count and recipe.use_ingredients is not None or is_ingredients_only) and all_ingredients_required and not (has_required_ingredients or crafter is not context.sim and cls.show_disabled_crafting_recipes):
                pass
            else:
                ...
                (original_price, discounted_price, ingredients_price) = (0, 0, 0)
                ...
                recipe_test_result = CraftingProcess.recipe_test(target, context, recipe, crafter, ingredients_price, paying_sim=context.sim, funds_source=funds_source, discounted_bucks_prices=discounted_bucks_prices)
                if not (subclass_of_order_interaction and (cls.ingredient_source and has_required_ingredients or is_ingredients_only) and cls.use_ingredients_default_value and funds_source is None and recipe_test_result.visible):
                    pass
                else:
                    row = RecipePickerRow(icon=recipe.icon_override, name=recipe.get_recipe_name(crafter), skill_level=recipe.required_skill_level, linked_recipe=recipe.base_recipe, display_name=recipe.get_recipe_picker_name(crafter), tag=recipe, tag_list=recipe.tuning_tags, mtx_id=recipe.entitlement, subrow_sort_id=recipe.subrow_sort_id, group_recipe_override=recipe.group_recipe_override, linked_recipe_override=recipe.linked_recipe_override, price=original_price, is_enable=enable_recipe & recipe_test_result.enabled, price_with_ingredients=ingredients_price, pie_menu_influence_by_active_mood=recipe_test_result.influence_by_active_mood, discounted_price=discounted_price, bucks_costs=bucks_costs)
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        for (i, recipe) in enumerate(recipes):
            if cls.recipe_tag not in recipe.recipe_tags:
                ...
                recipe_icon = IconInfoData(icon_resource=recipe.icon_override, obj_def_id=recipe.final_product_definition_id, obj_geo_hash=recipe.final_product_geo_hash, obj_material_hash=recipe.final_product_material_hash)
                row = RecipePickerRow(name=recipe.get_recipe_name(sim), icon=recipe.icon_override, row_description=recipe.recipe_description(sim), linked_recipe=recipe.base_recipe, display_name=recipe.get_recipe_picker_name(sim), icon_info=recipe_icon, tag=recipe, skill_level=i, group_recipe_override=recipe.group_recipe_override, linked_recipe_override=recipe.linked_recipe_override)
                yield row
