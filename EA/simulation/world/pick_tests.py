...
class PickTypeTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def __call__(self, context=None):
        ...
        if self.terrain_tags is not None and not terrain.is_terrain_tag_at_position(position.x, position.z, self.terrain_tags, level=pick_info.routing_surface.secondary_id):
