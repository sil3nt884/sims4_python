...
class ServiceManager:
    ...
    def pre_sim_info_load_options(self, options_proto):
        for service in self.services:
            ...
            try:
                with context:
                    service.pre_sim_info_load_options(options_proto)
                self._perf_log(context, PERF_FINISHED_MSG, service, 'pre_sim_info_load_options')
            except:
                logger.exception('Failed to load pre-sim-info options for {}', service)
                self._perf_log(context, PERF_ERROR_MSG, service, 'pre_sim_info_load_options')
