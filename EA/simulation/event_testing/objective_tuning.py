...
class BaseObjective:
    ...
    def increment_data(cls, resolver):
        return results.TestResultNumeric(False, "Objective doesn't have a tuned completion type.", current_value=0, goal_value=cls.goal_value(), is_money=cls.is_goal_value_money)
    ...
    @classmethod
    def apply_loot_on_completion_ui_update(cls, sim_info):
        pass
...
class Objective(BaseObjective, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.OBJECTIVE)):
    ...
    def apply_loot_on_completion_ui_update(cls, sim_info):
        resolver = SingleSimResolver(sim_info)
        for loot_action in cls.loot_on_completion_ui_update:
            loot_action.apply_to_resolver(resolver)
