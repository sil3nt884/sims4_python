...
class Photography:
    ...
    def create_photo_from_photo_data(cls, camera_mode, camera_quality, photographer_sim_id, target_obj_id, target_sim_ids, res_key, second_res_key, photo_style, photo_size, photo_orientation, photographer_sim_info, photographer_sim, time_stamp, selected_mood_param):
        ...
        reveal_level = PaintingState.REVEAL_LEVEL_MIN if photo_for_reference else PaintingState.REVEAL_LEVEL_MAX
        if camera_mode == CameraMode.PUZZLE_BY_REFERENCE:
            old_state = photo_object.canvas_component.painting_state
            canvas_state = PuzzleState.from_key(res_key, reveal_level=reveal_level, effect=photo_style, overlay_texture_id=old_state.overlay_texture_id, reveal_texture_id=old_state.reveal_texture_id, reveal_texture_id_b=old_state.reveal_texture_id_b)
        else:
            canvas_state = PaintingState.from_key(res_key, reveal_level=reveal_level, effect=photo_style)
        photo_object.canvas_component.painting_state = canvas_state
        photo_object.canvas_component.time_stamp = time_stamp
        photo_object.set_household_owner_id(photographer_sim.household_id)
        if camera_mode == CameraMode.PUZZLE_BY_REFERENCE:
            photo_object.linked_object_component.relink()
        if selected_mood_param:
            cls._apply_mood_category_state_if_appropriate(selected_mood_param, camera_mode, photo_object)
        if not photo_for_reference:
