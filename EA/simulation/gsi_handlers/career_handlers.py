...
def get_gig_history_data(sim_info):
    ...
    for gig_history in sim_info.career_tracker.gig_history.values():
        ...
        gig_history_data.append({'gig_title': gig_title, 'customer_name': gig_history.customer_name, 'customer_id': str(hex(gig_history.customer_id)), 'zone_id': str(hex(gig_history.lot_id)), 'gig_type': str(DecoratorGigLotType(gig_history.gig_lot_type)), 'gig_result': str(GigResult(gig_history.gig_result)), 'gig_score': str(gig_history.gig_score), 'gig_score_data': gig_score_data})
    return gig_history_data
