...
class _ObjectRoutingBehaviorBase(HasTunableFactory, AutoFactoryInit):
    ...
    def get_target_object(self):
        if self._target is not None and self._target.is_part:
...
class ObjectRouteFromFGL(_ObjectRoutingBehaviorBase):
    ...
    def get_routes_gen(self):
        ...
        terrain_tags = list(self.terrain_tags) if self.surface_type_override is not None and self.terrain_tags else []
        ...
        fgl_context = placement.create_fgl_context_for_object(starting_location, self._obj, min_distance=self.min_distance, max_distance=self.max_distance, terrain_tags=terrain_tags)
...
class TunableRoutingBehaviorConstraints(TunableGeometricConstraintVariant):
    ...
    def __init__(self, default='circle', **kwargs):
        super().__init__(routing_slot=TunableRoutingSlotConstraint(class_restrictions=ObjectAnimationElement), default=default, **kwargs)
...
class ObjectRouteFromTargetObject(_ObjectRoutingBehaviorBase):
            ...
            for part in target.parts:
                if self.target_selection_test:
                    ...
                    if not self.target_selection_test.run_tests(resolver):
                        pass
                    elif not (self.target_part.subroot_index is not None and part.subroot_index is None):
                        if part.subroot_index != self.target_part.subroot_index:
                            pass
                        else:
                            if self.requires_target_reservation():
                                if part.may_reserve(self._obj):
                                    return part
                            return part
                elif not (self.target_part.subroot_index is not None and part.subroot_index is None):
                    if part.subroot_index != self.target_part.subroot_index:
    ...
    def _find_target(self):
            ...
            for target in targets_source:
                if self.radius is not None:
                    ...
                    if dist_sq > self.radius:
                        ...
                        if target.is_sim or self.target_part or is_target_reserved:
                            pass
                        elif self.target_part is None:
                            if self.target_selection_test:
                                resolver = DoubleObjectResolver(self._obj, target)
                                if not self.target_selection_test.run_tests(resolver):
                                    pass
        ...
        def _try_reserve_object(obj):
            if obj.is_sim or obj.is_terrain:
                return
            if self.requires_target_reservation() and self.target_reservation_timing == TargetReservationTiming.ROUTING_BEHAVIOR:
                self._reservation_handler = obj.get_reservation_handler(self._obj)
                if self._reservation_handler and self._reservation_handler.may_reserve():
        ...
        if self.target_participant is not None:
            ...
            objects = [target.get_sim_instance() if not target is not None or not target.is_sim or target.is_instanced() else target for target in objects]
        ...
        if not dest_handles:
            ...
            if not connections:
                if allow_caching and routing_service.clear_cache_for_type(cache_key) and not objs_set_on_current_run:
    ...
    def do_target_action_rules_gen(self, timeline, route_successful):
        if self.target_action_rules.actions and self._target is None:
            return
        if self.target_action_rules.cancel_if_route_incomplete and not route_successful:
            return
    ...
    def _on_target_changed(self, child, location=None, **kwargs):
        if child is self._obj and location is None:
            self.release_target()
...
class ObjectRouteFromCreatedObject(ObjectRouteFromTargetObject):
    FACTORY_TUNABLES = {'create_route_target': ObjectCreation.TunableFactory(), 'locked_args': {'target_participant': ParticipantType.Invalid, 'target_selection_test': None, 'target_type': None, 'target_part': None}}
