...
class PaintingTexture(metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.RECIPE)):
    ...
    def _tuning_loaded_callback(cls):
        if cls.texture:
            if cls.for_puzzles:
                cls._base_canvas_state = PuzzleState.from_key(cls.texture)
            else:
                cls._base_canvas_state = PaintingState.from_key(cls.texture)
        else:
            cls._base_canvas_state = None

    @classmethod
    def apply_to_object(cls, obj):
        obj.canvas_component.painting_state = cls._base_canvas_state
